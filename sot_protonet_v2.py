# This code is modified from https://github.com/DanielShalam/SOT/blob/main/self_optimal_transport/self_optimal_transport.py
...
class SOT_ProtoNet_v2(MetaTemplate):
    def __init__(self, backbone, n_way, n_support):
        super(SOT_ProtoNet_v2, self).__init__(backbone, n_way, n_support)
        self.sot_layer = SOT()  # Initialize the SOT layer
        self.loss_fn = nn.CrossEntropyLoss()

    def set_forward(self, x):
        # Apply SOT layer to the entire feature set
        z_transformed = self.sot_layer(x)

        # Split into support and query sets
        z_support = z_transformed[:self.n_way * self.n_support]
        z_query = z_transformed[self.n_way * self.n_support:]
        ...
        print("-"*50)
        print("z transform shape : ", z_transformed.shape)
        print("z support : ", z_support.shape)
        print("z proto shape : ", z_proto.shape)
        print("z query shape : ", z_query.shape)
        print("-"*50)
...
here is the output :
--------------------------------------------------
z transform shape :  torch.Size([5, 20, 20])
z support :  torch.Size([5, 20, 20])
z proto shape :  torch.Size([5, 80])
z query shape :  torch.Size([0, 20, 20])
--------------------------------------------------

  File "/home/kenjitetard0/bio-v2-1/fewshotbenchv2/methods/sot_protonet_v2.py", line 196, in set_forward
    dists = self.euclidean_dist(z_query, z_proto)
  File "/home/kenjitetard0/bio-v2-1/fewshotbenchv2/methods/sot_protonet_v2.py", line 203, in euclidean_dist
    return torch.pow(x - y, 2).sum(2)
RuntimeError: The size of tensor a (20) must match the size of tensor b (80) at non-singleton dimension 2
